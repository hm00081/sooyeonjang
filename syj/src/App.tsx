  ...
  useEffect(() => {
    ...
    const orders = {
      name: d3.range(n).sort(function (a, b) {
        return d3
        .ascending(nodes[a].name, nodes[b].name); //1~440까지 ascending
      }),
    ...
    function rowFunction(row: NodeCycle[], rowIndex: number) {
      ...
      Selection.each(function (nodeCycle) {
        ...
        for (let cycle = 0; cycle < cc[0].length; cycle++) {
          if (
            cc[rowIndex][cycle] === 0 &&
            cc[nodeCycle.nodeIndex][cycle] === 0
          ) {
            // fill(none)
          } else if (
            cc[rowIndex][cycle] === 0 &&
            cc[nodeCycle.nodeIndex][cycle] === 1
          ) {
            // fill(none)
          } else if (
            cc[rowIndex][cycle] === 1 &&
            ...
            if (selectVariable === "0" || selectVariable === "1" || selectVariable === "2" || selectVariable === "3" || selectVariable === "4" || selectVariable === "5" || selectVariable ===  "6" || selectVariable === "7" ) { //각 사이클
        ...
        d3.select(this)
          ...
          .attr("d", (d, i) => {
            ...
            if (cycles.length === 1) {
              ...
              attribute = `M ${x(nodeCycle.x)} ${x(nodeCycle.y)} H ${x(nodeCycle.x)! + x.bandwidth()
                } V ${x(nodeCycle.y)! + x.bandwidth()} H${x(nodeCycle.x)} L${x(
                  nodeCycle.x
                )} ${x(nodeCycle.y)}`;
            }
            if (cycles.length === 2) {
              if (i === 0) {
                // first path
                attribute = `M ${x(nodeCycle.x)! + x.bandwidth()} ${x(
                  nodeCycle.y
                )} L ${x(nodeCycle.x)} ${x(nodeCycle.y)! + x.bandwidth()} L${x(nodeCycle.x)! + x.bandwidth()
                  } ${x(nodeCycle.y)! + x.bandwidth()} Z`;
              } else if (i === 1) {
                // second path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)} L ${x(
                  nodeCycle.x
                )} ${x(nodeCycle.y)! + x.bandwidth()} L${x(nodeCycle.x)! + x.bandwidth()
                  } ${x(nodeCycle.y)} Z`;
              }
            } else if (cycles.length === 3) {
              if (i === 0) {
                // first path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)! + x.bandwidth()
                  } H ${x(nodeCycle.x)! + x.bandwidth()} V ${x(nodeCycle.y)! + (x.bandwidth() * 2) / 3
                  } H ${x(nodeCycle.x)}Z`;
              } else if (i === 1) {
                // second path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)! + (x.bandwidth() * 2) / 3
                  } H ${x(nodeCycle.x)! + x.bandwidth()} V ${x(nodeCycle.y)! + x.bandwidth() / 3
                  } H ${x(nodeCycle.x)}Z`;
              } else if (i === 2) {
                // third path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)! + x.bandwidth() / 3
                  } H ${x(nodeCycle.x)! + x.bandwidth()} V ${x(
                    nodeCycle.y
                  )} H ${x(nodeCycle.x)} Z`;
              }
              ...
              if (i === 0) {
                // first path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)} L${x(nodeCycle.x)! + x.bandwidth() / 2
                  } ${x(nodeCycle.y)! + x.bandwidth()}L${x(nodeCycle.x)! + x.bandwidth()
                  } ${x(nodeCycle.y)}Z`;
              } else if (i === 1) {
                // second path
                attribute = `M${x(nodeCycle.x)! + x.bandwidth()} ${x(nodeCycle.y)! + x.bandwidth()
                  }L${x(nodeCycle.x)! + x.bandwidth() / 2} ${x(nodeCycle.y)! + x.bandwidth() / 2
                  } L${x(nodeCycle.x)! + x.bandwidth()} ${x(nodeCycle.y)}Z`;
              } else if (i === 2) {
                // third path
                attribute = `M${x(nodeCycle.x)! + x.bandwidth() / 2} ${x(nodeCycle.y)! + x.bandwidth() / 2
                  }L${x(nodeCycle.x)}  ${x(nodeCycle.y)! + x.bandwidth()}L${x(nodeCycle.x)! + x.bandwidth()
                  } ${x(nodeCycle.y)! + x.bandwidth()} Z`;
              } else if (i === 3) {
                // fourth path
                attribute = `M${x(nodeCycle.x)} ${x(nodeCycle.y)} L${x(
                  nodeCycle.x
                )} ${x(nodeCycle.y)! + x.bandwidth()} L${x(nodeCycle.x)! + x.bandwidth() / 2
                  } ${x(nodeCycle.y)! + x.bandwidth() / 2}Z`;
              }
            }
        ...
        <div style={{ display: "flex" }}>
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#7dbbff", paddingRight: "10em" }}
            ></rect>
          </svg>
          <div>Cycle 1</div>
          ...
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#adc5a7", paddingRight: "10em" }}
            ></rect>
          </svg>
          <div>Cycle 2</div>
          ...
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#c4ca7a", paddingRight: "10em" }}
            ></rect>
          </svg>
          <div>Cycle 3</div>
          ...
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#dccf4e", paddingRight: "10em" }}
            ></rect>
          </svg>
          <div>Cycle 4</div>
        </div>
        ...
        <div>
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#f3d422", paddingRight: "10em" }}
            ></rect>
          </svg>
          Cycle 5
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#ff9c09", paddingRight: "10em" }}
            ></rect>
          </svg>
          Cycle 6
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#ff4e04", paddingRight: "10em" }}
            ></rect>
          </svg>
          Cycle 7
          <svg width="13" height="13" style={{ marginLeft: 7 }}>
            <rect
              width="13"
              height="13"
              style={{ fill: "#ff0000", paddingRight: "10em" }}
            ></rect>
          </svg>
          Cycle 8
        </div>
